...
export class ScriptLoader {
    ...
    private async findFileInWorkspace(
        ...
        for (const file of files) {
            const relativePath = path
                .relative(this.workspaceRoot, file.fsPath)
                .replace(/\\/g, '/')
                .toLowerCase();
    ...
    private extractDependencies(content: string, scriptPath: string): string[] {
        ...
        const regex =
            /^(?!.*--.*$).*?Script\.ReloadScript\s*(?:\(\s*)?["']([^"']+)["']\s*(?:\))?\s*;?\s*\n?/gm;
        let match;

        this.log(`Extracting dependencies from: ${path.basename(scriptPath)}`);
        while ((match = regex.exec(content)) !== null) {
            this.log(
                `Found dependency in ${path.basename(scriptPath)}: ${match[0]}`
            );
            const dep = match[1].replace(/\\/g, '/');
            dependencies.push(dep);
            this.log(
                `Found dependency in ${path.basename(scriptPath)}: ${dep}`
            );
        }
    ...
    private async processScript(
        scriptPath: string,
        accumulator: ScriptInfo[],
        content?: string
    ): Promise<void> {
        if (this.processedScripts.has(scriptPath)) {
            this.log(
                `Script already processed, skipping: ${path.basename(
                    scriptPath
                )}`
            );
            return;
        ...
        if (scriptInfo.dependencies.length > 0) {
            this.log(
                `Processing dependencies for ${path.basename(scriptPath)}:`
            );
        }
